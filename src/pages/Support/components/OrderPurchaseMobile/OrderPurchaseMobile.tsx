...
export default function OrderPurchaseMobile({ purchase }: Props) {
  ...
  return (
    <div className='grid grid-cols-4 px-1 py-2 text-sm font-medium text-textDark dark:text-textLight sm:py-3'>
      ...
      <div className='col-span-3 flex w-full flex-col justify-between pl-3'>
        <div className='flex items-center justify-between'>
          <p className='w-full text-sm font-medium sm:text-base'>{purchase.item.name}</p>
          <span className='text-xs font-medium sm:text-sm'>${purchase.item.price}</span>
        </div>
        <div className='flex w-full items-center justify-between text-xs sm:text-sm'>
          <span className='capitalize text-textDark/60 dark:text-textLight/60'>{purchase.item.color}</span>
          <span className='lowercase'>x{purchase.quantity}</span>
        </div>
        <div className='w-full text-right text-sm font-medium text-haretaColor sm:text-base'>
