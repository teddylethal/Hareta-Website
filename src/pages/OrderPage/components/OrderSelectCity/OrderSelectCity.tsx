...
import { AddressCity } from 'src/types/location.type'

interface Props {
  isError: boolean
}

export default function OrderSelectCity({ isError }: Props) {
  const { theme } = useContext(AppContext)
  const { addressCountry, addressState, addressCity, setAddressCity } = useContext(OrderContext)

  //! Get state list list
  const { data: citiesData } = useQuery({
    queryKey: ['cities', addressCountry?.iso2, addressState?.iso2],
    queryFn: () => locationApi.getCitiesByStateAndCountry(addressCountry?.iso2 || '', addressState?.iso2 || ''),
    staleTime: 1000 * 60 * 60 * 24,
    enabled: Boolean(addressCountry) && Boolean(addressState)
  })
    ...
    <div className='w-full '>
      <div className='flex w-full cursor-not-allowed items-center justify-center rounded-xl bg-lightColor900 outline-haretaColor/60 hover:outline-haretaColor dark:bg-darkColor900'>
        {(!addressCountry || !addressState) && (
          <div className='flex h-10 items-center justify-center opacity-60'>{t('bill.City')}</div>
        )}
        {addressCountry && addressState && !citiesData && <LoadingRing width={40} />}
      </div>
      {citiesData && (
        <Combobox value={addressCity} onChange={setAddressCity} onClose={() => setQuery('')}>
          <div className='relative'>
            <ComboboxInput
              ...
              displayValue={(element: AddressCity) => element?.name || ''}
